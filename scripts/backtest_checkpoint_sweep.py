"""Sweep checkpoint_cut threshold to find optimal value.

The checkpoint_cut gate is the #1 P&L destroyer (-$5,562 in backtest).
Tests: what if we raise the threshold or disable it entirely?
"""
...
from options_owl.risk.exit_v5.config import V5Config, AdaptiveTier, categorize_ticker
    ...
    for idx in range(1, len(df)):
        ...
        if action.should_exit:
            ...
            return {"pnl": pnl, "reason": action.reason.value,
                    "hold": (now - entry_ts).total_seconds() / 60}
...
def main():
    ...
    # Sweep configs
    configs = {
        "default (cp=30%)": V5Config(),
        "cp=40%": V5Config(checkpoint_drop_pct=40.0),
        "cp=45%": V5Config(checkpoint_drop_pct=45.0),
        "cp=50%": V5Config(checkpoint_drop_pct=50.0),
        "cp=60%": V5Config(checkpoint_drop_pct=60.0),
        "no_cp (disabled)": V5Config(checkpoint_drop_pct=100.0),
        # Also try: wider underlying threshold
        "cp=30% u=1.0%": V5Config(underlying_against_threshold=1.0),
        "cp=30% u=1.5%": V5Config(underlying_against_threshold=1.5),
        # Tighter graduated stop + no checkpoint
        "no_cp, tight_stop=30%": V5Config(checkpoint_drop_pct=100.0, tight_stop_0dte_pct=30.0),
        # Soft trail keep variants
        "keep=70%": V5Config(soft_trail_keep_pct=0.70),
        "keep=75%": V5Config(soft_trail_keep_pct=0.75),
        # Combined: no checkpoint, tighter stop, keep 70%
        "no_cp+tight30+keep70": V5Config(
            checkpoint_drop_pct=100.0, tight_stop_0dte_pct=30.0, soft_trail_keep_pct=0.70
        ),
        # Wider soft trail band (protect gains earlier)
        "keep70+band_low=5%": V5Config(soft_trail_keep_pct=0.70, soft_trail_band_low_pct=5.0),
    }

    print(f"{'Config':<25} {'P&L':>9} {'Win%':>6} {'W/L':>8} "
          f"{'CP exits':>8} {'CP P&L':>9} {'AvgHold':>8}")
    print("-" * 85)

    for name, cfg in configs.items():
        total_pnl = 0
        wins = losses = cp_exits = 0
        cp_pnl = 0
        total_hold = 0
        ...
        print(f"{name:<25} ${total_pnl:>7,.0f} {wr:>5.1f}% {wins:>3}/{losses:<3} "
              f"{cp_exits:>8} ${cp_pnl:>7,.0f} {avg_hold:>5.0f}m")

    # ── Detailed: show what happens to checkpoint trades with different configs
    print(f"\n{'=' * 90}")
    print("CHECKPOINT TRADES DETAIL — what happens when we disable checkpoint?")
    print(f"{'=' * 90}")

    # Find trades that exit via checkpoint in default config
    default_cfg = V5Config()
    no_cp_cfg = V5Config(checkpoint_drop_pct=100.0)

    print(f"\n{'Day':<12} {'Ticker':<6} {'Dir':<5} {'Default$':>9} {'Default Reason':<18} "
          f"{'NoCp$':>9} {'NoCp Reason':<18} {'Delta':>8}")
    print("-" * 100)
    ...
    for td in trade_data:
        ...
        if r_default["reason"] != "checkpoint_cut":
            continue
        r_nocp = simulate(
            td["df"], td["entry_premium"], td["contracts"],
            td["direction"], td["dte"], td["expiry_date"], td["ticker"], no_cp_cfg,
        )
        delta = r_nocp["pnl"] - r_default["pnl"]
        print(f"{td['day']:<12} {td['ticker']:<6} {td['direction'][:4]:<5} "
              f"${r_default['pnl']:>7,.0f} {r_default['reason']:<18} "
              f"${r_nocp['pnl']:>7,.0f} {r_nocp['reason']:<18} ${delta:>+7,.0f}")
