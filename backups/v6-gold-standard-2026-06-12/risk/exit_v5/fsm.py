...
class ExitFSM:
    ...
    def __init__(self, cfg: V5Config, settings=None):
        ...
        self._regime_tighten_factor: float = 1.0  # 1.0 = no tightening

    def apply_regime_tighten(self, factor: float = 0.60) -> None:
        """Tighten adaptive trail widths by factor (0.6 = 40% tighter).

        Called by position_monitor when regime flips against open positions.
        Stacks multiplicatively with 2PM tightening.
        Reset to 1.0 when regime re-aligns.
        """
        self._regime_tighten_factor = factor

    def clear_regime_tighten(self) -> None:
        """Remove regime-based trail tightening."""
        self._regime_tighten_factor = 1.0
    ...
    def evaluate(
        ...
        # Compute combined tighten factor (2PM + regime, multiplicative)
        combined_tighten = 1.0
        soft_boost = 0.0

        if (self._settings
                and getattr(self._settings, "ENABLE_V6_2PM_TIGHTEN", False)
                and now_et.hour >= 14):
            combined_tighten *= getattr(self._settings, "V6_2PM_TRAIL_TIGHTEN_FACTOR", 0.7)
            soft_boost += getattr(self._settings, "V6_2PM_SOFT_TRAIL_BOOST", 0.15)

        # Regime-triggered tightening (spec 08) — stacks with 2PM
        if self._regime_tighten_factor < 1.0:
            combined_tighten *= self._regime_tighten_factor
            debug["regime_tighten"] = round(self._regime_tighten_factor, 2)

        if combined_tighten < 1.0:
            from dataclasses import replace
            base_tiers = cfg.get_adaptive_tiers(state.category)
            tight_tiers = tuple(
                AdaptiveTier(t.min_peak_gain, t.trail_width * combined_tighten)
                for t in base_tiers
            )
            new_keep = min(0.80, cfg.soft_trail_keep_pct + soft_boost)
